...
$pdf->Cell(0,2,"RELATORIO DE CLIENTES EM ATRASO : ".$ano ,0,2,'C'); 

		$pdf->Cell(6,1,'Nome' ,1,0,'C');
		$pdf->Cell(3,1,'Data Pag.' ,1,0,'C');
		$pdf->Cell(1.5,1,'Status' ,1,0,'C');
...
where year(parcela.dt_vencimento) = '$ano' and month(parcela.dt_vencimento) = '$mes' and parcela.`pago`= 'Atraso' ";
...
$data = implode("-", array_reverse(explode("-",$data)));
		...
		$pdf->Cell(1.5,1,$resultado['pago'] ,1,0,'C');
		...
		$pdf->Cell(18.5,1,'Total                                                                                                                                     R$'.$soma,1,1,'L');
